...
column_name = 'indcode'
unique_values = data[column_name].unique() # Get the unique values in the specified column of the data

# Filter the unique values to only include those that are longer than 2 characters and do not contain a hyphen
unique_values = [value for value in unique_values if len(str(value)) > 2 and '-' not in str(value)] 

for value in unique_values:
    filtered_data = data[data[column_name] == value] # Filter the data to only include rows where the specified column is equal to the current unique value
    file_name = f"{value}.xlsx" # Construct the file name for the output file
    filtered_data.to_excel(file_name,index=False) # Save the filtered data to an Excel file