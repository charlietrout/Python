...
data = {
    "Year": [year]
}  # Initialize a dictionary with a 'Year' key and the specified year as the value
for county, population in zip(
    county_names, population_numbers
):  # Iterate over each county name and population number
    data[county] = [
        population
    ]  # Add a new key-value pair to the dictionary for the current county and population
...
if (
    ...
    for row in ws.iter_rows(
        values_only=True
    ):  # Iterate over each row in the worksheet, returning only the cell values
        data_rows.append(row)  # Append the row to the data_rows list
    if data_rows:
        old_df = pd.DataFrame(
            data_rows[1:], columns=data_rows[0]
        )  # Create a DataFrame from the data, using the first row as column headers
    else:
...
for r_index, row in enumerate(dataframe_to_rows(combined_df, index=False, header=True)):
    for c_index, value in enumerate(row):
        ws.cell(
            row=r_index + 1, column=c_index + 1, value=value
