...
data = {'Year': [year]} # Initialize a dictionary with a 'Year' key and the specified year as the value
for county, population in zip(county_names, population_numbers): # Iterate over each county name and population number
    data[county] = [population] # Add a new key-value pair to the dictionary for the current county and population
new_df = pd.DataFrame(data) # Create a new DataFrame from the dictionary
...
if sheet_name in book.sheetnames: # Check if the specified sheet name exists in the workbook
    ...
    data_rows = [] # Initialize an empty list to store the data rows
    for row in ws.iter_rows(values_only=True): # Iterate over each row in the worksheet, returning only the cell values
        data_rows.append(row) # Append the row to the data_rows list
    if data_rows:
        old_df = pd.DataFrame(data_rows[1:], columns=data_rows[0]) # Create a DataFrame from the data, using the first row as column headers
    else:
        old_df = pd.DataFrame() # Create an empty DataFrame
else:
    old_df = pd.DataFrame()
