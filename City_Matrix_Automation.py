...
def paste_to_excel(df, file, worksheet, row, col, header=False):
    ...
    with pd.ExcelWriter(
        ...
        df.to_excel(
            writer, worksheet, index=True, header=header, startrow=row, startcol=col
        )
...
fips_to_place = {
    ...
    "01360": "Anderson city",
    "07210": "Bluffton town",
    "13330": "Charleston city",
    "16000": "Columbia city",
    "16405": "Conway city",
    "25810": "Florence city",
    "26890": "Fort Mill town",
    "29815": "Goose Creek city",
    "30850": "Greenville city",
    "30985": "Greer city",
    "34045": "Hilton Head Island town",
    "45115": "Mauldin city",
    "48535": "Mount Pleasant town",
    "49075": "Myrtle Beach city",
    "50875": "North Charleston city",
    "61405": "Rock Hill city",
    "68290": "Spartanburg city",
    "70270": "Summerville town",
    "70405": "Sumter city",
}
...
for col, place in enumerate(sc_places):
    ...
    place_data_2010 = data_2012.query(f"place == '{fips_code}'")["S1810_C01_001E"].iloc[
        0
    ]
    # Append the data to the list for 2010
    place_data_list_2010.append(float(place_data_2010))

    place_data_2014 = data_2016.query(f"place == '{fips_code}'")["S1810_C01_001E"].iloc[
        0
    ]
    place_data_list_2014.append(float(place_data_2014))

    place_data_2019 = data_2021.query(f"place == '{fips_code}'")["S1810_C01_001E"].iloc[
        0
    ]
...
paste_to_excel(
    df=merged_df, file="city_file.xlsx", worksheet="Sheet1", row=0, col=0, header=True
)
