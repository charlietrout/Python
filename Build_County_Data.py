...
series = ['LAUCN450910000000006','LAUCN450910000000005','LAUCN450910000000004','LAUCN450910000000003']
...
xl = pandas.ExcelFile(config['file_path'])
...
df2 = df2[['Date', 'Abbeville County', 'Aiken County', 'Allendale County', 'Anderson County', 'Bamberg County', 'Barnwell County', 'Beaufort County', 'Berkeley County', 'Calhoun County', 
           'Charleston County', 'Cherokee County', 'Chester County', 'Chesterfield County', 'Clarendon County', 'Colleton County', 'Darlington County', 'Dillon County', 'Dorchester County',
            'Edgefield County', 'Fairfield County', 'Florence County', 'Georgetown County', 'Greenville County', 'Greenwood County', 'Hampton County', 'Horry County', 'Jasper County', 
            'Kershaw County', 'Lancaster County', 'Laurens County', 'Lee County', 'Lexington County', 'McCormick County','Marion County','Marlboro County','Newberry County','Oconee County',
            'Orangeburg County','Pickens County','Richland County','Saluda County','Spartanburg County','Sumter County','Union County','Williamsburg County','York County','State #']]

# Convert the 'Date' column to a datetime object with the desired format
df2['Date'] = pandas.to_datetime(df2['Date'], format=input_format)

# Melt df2 to transform the county columns into rows
...
# Add a new column named 'emppopratio' to merged_df2 that divides the values in the 'emplab' column by the values in the 'Population' column and multiplies the result by 100
merged_df2 = merged_df2.assign(County_Ratio=merged_df2['emplab'] / merged_df2['CNP'])
...
# Rename the 'Area' column to 'Areaname'
merged_df2 = merged_df2.rename(columns={'Area': 'Areaname'})

merged_df2 = merged_df2.rename(columns={'Ratio': 'State CNP/County CNP Ratio'})
...
merged_df2 = merged_df2.rename(columns={'County_Ratio': 'County Ratio'})

merged_df2 = merged_df2.assign(LFPR=merged_df2['LF/Adjusted']*100)

# Define the desired column order
column_order = ['Areaname', 'Date', 'laborforce', 'emplab', 'unemp', 'unemprate', 'emppopratio', 'County Ratio', 'Population', 'State Population', 'State CNP/County CNP Ratio', 'CNP', 'LF/Adjusted', 'LFPR']
