...
data_2012 = get_census_county_data(
    year=acs_years[0], dname="acs5", cols="S1810_C01_001E", s_path="subject", county="*"
)

# Retrieve the data for the year 2016
data_2016 = get_census_county_data(
    year=acs_years[1], dname="acs5", cols="S1810_C01_001E", s_path="subject", county="*"
)

# Retrieve the data for the year 2021
data_2021 = get_census_county_data(
    year=acs_years[2], dname="acs5", cols="S1810_C01_001E", s_path="subject", county="*"
)
...
for col, county in enumerate(sc_counties):
    ...
    county_data_2010 = data_2012.query(f"county == '{fips_code}'")[
        "S1810_C01_001E"
    ].iloc[0]
    # Append the data to the list for 2010
    county_data_list_2010.append(float(county_data_2010))

    county_data_2014 = data_2016.query(f"county == '{fips_code}'")[
        "S1810_C01_001E"
    ].iloc[0]
    county_data_list_2014.append(float(county_data_2014))

    county_data_2019 = data_2021.query(f"county == '{fips_code}'")[
        "S1810_C01_001E"
    ].iloc[0]
...
paste_to_excel(
    df=merged_df, file="file.xlsx", worksheet="Sheet1", row=0, col=0, header=True
)
